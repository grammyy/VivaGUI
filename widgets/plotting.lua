viva.registerWidgets({
    {
        ...
        function(window,self,stack) --add height style
            ...
            for i,value in pairs(self.table) do
                ...
                render.drawLine(stack.x+((width/(#self.table-1))*(i-1)),stack.y+7.75-math.clamp(value,-7.75,7.75),stack.x+((width/(#self.table-1))*(i)),stack.y+7.75-math.clamp(self.table[i+1],-7.75,7.75))
                --math slightly off ^
            end
